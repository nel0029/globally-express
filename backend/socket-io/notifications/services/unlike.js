...
const unlike = async (data, socket, io) => {
    ...
    if (likeExists) {
        const deletedNotification = await Notifications.findOneAndDelete({ actionID: likeExists._id, actorID: actorID })
        const unseenNotifications = await UnseenNotifications.findOne({ targetID: targetID })
