...
const { Messages, Conversations, ConversationMembers, UnseenMessagesCount } = require('../../../models/messageModel');
const { ActiveUsers } = require('../../../models/userModel');

const createNewMessage = async (data, socket, io) => {
    const { senderID, receiverID, text } = data

    const conversationSenderExist = await ConversationMembers.findOne({ senderID: senderID, receiverID: receiverID })
    ...
    if (!conversationSenderExist) {
        ...
        if (newConversation) {
            ...
            const isReceiverActive = await ActiveUsers.findOne({ userID: newSenderConversationMember.receiverID })
            ...
            if (isReceiverActive) {
                ...
                io.to(isReceiverActive.socketID).emit("receiveNewMessages", receiverNewConvo[0])
