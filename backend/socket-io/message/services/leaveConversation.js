...
const leaveConversation = async (data, socket, io) => {
    ...
    if (conversationExists) {
        const memberExists = await ConversationMembers.findOne({ conversationID: conversationExists._id, memberID: memberID })

        if (memberExists) {
            socket.leave(memberExists.conversationID)
            console.log("Leave at:", memberExists.conversationID)
        }
    }
}
