...
const sendMessage = async (data, socket, io) => {
  ...
  const conversationExists = await Conversations.findOne({
    $and: [{ _id: conversationID }, { senderID: senderID }],
  });

  if (conversationExists) {
    const senderMembershipExists = await ConversationMembers.findOne({
      $and: [
    ...
    if (senderMembershipExists) {
      ...
      const isSenderActive = await ActiveUsers.findOne({
        userID: senderMembershipExists.senderID,
      });
      const isReceiverActive = await ActiveUsers.findOne({
        userID: senderMembershipExists.receiverID,
      });
      ...
      if (isReceiverActive) {
        ...
        io.to(isSenderActive.socketID).emit("receiveMessage", forSender);
      } else {
        io.to(isSenderActive.socketID).emit("receiveMessage", forSender);
      }
    }
