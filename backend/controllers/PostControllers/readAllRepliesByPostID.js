...
const readAllRepliesByPostID = asyncHandler(async (req, res) => {
    ...
    const author = await Users.find({ userName: userName }).lean().exec()
    const parentPostID = Types.ObjectId(postID)
    ...
    if (author) {
        ...
        res.status(404).json({ message: `This user who has a userName: ${userName} did not exists` })
    }
...
module.exports = readAllRepliesByPostID